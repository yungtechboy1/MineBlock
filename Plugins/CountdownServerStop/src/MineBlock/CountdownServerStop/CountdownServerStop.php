<?php
namespace MineBlock\CountdownServerStop;
...
class CountdownServerStop extends PluginBase{
	...
	public function countdown($t, $tt = false){
		...
		if($t - $tt < 1){
			...
			$tt++;
			$this->getServer()->getScheduler()->scheduleDelayedTask(new CallbackTask([$this,"broadCast"], [$this->isKorean() ? "[CountDownServerStop] 서버가 " . ($t - $tt) . " 초 후에 종료됩니다.": "[CountDownServerStop] Server is stop to ".($t - $tt)]), 20);
