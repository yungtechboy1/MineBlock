...
class Login extends PluginBase implements Listener{
	...
	public function onTick(){
		foreach($this->getServer()->getOnlinePlayers() as $p){
			if(!$this->isLogin($p)){
				if(!isset($this->spawn[$n = strtolower($p->getName())])) $this->spawn[$n] = $p->getPosition();
	...
	public function sendLogin($p, $l = false){
		if($p instanceof Player){
			...
			$ik = $this->isKorean();
			$n = strtolower($p->getName());
			if($this->isLogin($p)){}elseif(!isset($this->lg[$n])){
	...
	public function loadYml(){
		...
		$this->lg = (new Config($this->getServer()->getDataPath() . "/plugins/! MineBlock/" . "Login.yml", Config::YAML))->getAll();
	}

	public function saveYml(){
		ksort($this->lg);
		$lg = new Config($this->getServer()->getDataPath() . "/plugins/! MineBlock/" . "Login.yml", Config::YAML);
		$lg->setAll($this->lg);
		$lg->save();
	}
