...
class CocoaBean extends PluginBase implements Listener{
	...
	public function loadBeanYml(){
		...
		$this->cb = (new Config($this->getServer()->getDataPath() . "/plugins/! MineBlock/" . "CocoaBean.yml", Config::YAML, []))->getAll();
	}
	...
	public function saveBeanYml(){
		...
		ksort($cb);
		$cbc = new Config($this->getServer()->getDataPath() . "/plugins/! MineBlock/" . "CocoaBean.yml", Config::YAML, []);
		$cbc->setAll($cb);
		$cbc->save();
		$this->cb = $cb;
	}
