...
class MineFarm extends PluginBase implements Listener{
	...
	public function onBlockEvent($event, $isBreak = false, $isTouch = false){
		...
		if($isTouch){
			...
			if(isset($this->sh[$this->getPos($b)])){
				if(!$p->hasPermission("mineblock.shop.block")) $event->setCancelled();
			}
		}else{
		...
		if(strtolower($b->getLevel()->getFolderName()) == strtolower($this->mf["MineWorld"]) && !$p->hasPermission("mineblock.minefarm.block")){
		...
		if($event->isCancelled() || strtolower($p->getLevel()->getName()) !== $this->name || $p->hasPermission("mineblock.minefarm.block")) return;
		if($this->isFarm($b)){
			if(!$this->isOwn($p, $b) && !$this->isShare($p, $b)){
				...
				foreach($drops as $drop){
					if($drop[2] <= 0) continue;
					 elseif($p instanceof Player && $p->isSurvival()) $p->getInventory()->addItem(Item::get(...$drop));
					else continue;
				}
			}elseif($isTouch){
	...
	public function onTick(){
		...
		foreach($this->getServer()->getOnlinePlayers() as $p){
			...
			if(!isset($this->move[$n = strtolower($p->getName())])) $this->move[$n] = $p->getPosition();
			$isTp = true;
			if(strtolower($p->getLevel()->getName()) == $this->name && !$p->hasPermission("mineblock.minefarm.move") && (!$this->isFarm($p) || !$this->isOwn($p, $p) && !$this->isInvite($p, $p))){
	...
	public function addShop($pos, $mode, $id, $cnt, $pr){
		...
		$pos[3] = $l;
		 if($l != false) $l->setBlock(new Position(...$pos),Block::get(20));
		return true;
	}
