...
class ItemCase extends PluginBase implements Listener{
	...
	public function onBlockEvent($event, $isTouch = false){
		...
		if(isset($this->ic[$this->getPos($event->getBlock())])){
			...
			if(!$p->hasPermission("mineblock.itemcase.block")) $event->setCancelled();
			if($p->hasPermission("mineblock.itemcase.spawn")) $this->spawnCase(true);
		}
	}
	...
	public function loadYml(){
		...
		$this->ic = (new Config($this->getServer()->getDataPath() . "/plugins/! MineBlock/" . "ItemCase.yml", Config::YAML))->getAll();
	}

	public function saveYml(){
		ksort($this->ic);
		$ic = new Config($this->getServer()->getDataPath() . "/plugins/! MineBlock/" . "ItemCase.yml", Config::YAML);
		$ic->setAll($this->ic);
		$ic->save();
	}
