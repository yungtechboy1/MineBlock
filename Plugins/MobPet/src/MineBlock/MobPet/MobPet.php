...
class MobPet extends PluginBase implements Listener{

	public function onEnable(){
		$this->getServer()->getPluginManager()->registerEvents($this, $this);
//		Entity::registerEntity(MobPetEntity::class);
		$this->player = [];
		$spawnEgg = [15,10,11,12,13,14,16,33,38,39,34,35,37,39,36];
		foreach($spawnEgg as $se) Block::$creative[] =	[383, $se];
	}

	public function onPlayerMove(PlayerMoveEvent $event){
		if(!isset($this->player[$n = strtolower($event->getPlayer()->getName())]) || $this->player[$n]->dead){
			$this->player[$n] = new MobPetEntity($event->getPlayer(), 6, 0, 1);
		}
	}
}

class MobPetEntity extends Entity{
	const NETWORK_ID = 10;
	...
	public function __construct(Entity $target, $distance = 0, $tailCount = 0, $tailDistance = null){
		parent::__construct($target->getLevel()->getChunk($target->x >> 4, $target->z >> 4), new Compound("", [
			"Pos" => new Enum("Pos", [
				new Double("", $target->x),
				new Double("", $target->y + 0.5),
				new Double("", $target->z),
			]),
		...
		if($tailCount > 0) new MobPetEntity($this, $distance, $tailCount-1, $tailDistance);
		...
		if($this->closed == true || $this->dead == true || ($target = $this->target) == null || $target->dead || $this->player instanceof Player && !$this->player->spawned){
			...
			if($this->isHead){
				...
				if($this->isFallow && $dis > 20){
					...
					if($target instanceof Player) $target->sendMessage("[MobPet] 같이가요 ");
					return true;
	...
	public function attack($damage, $source = EntityDamageEvent::CAUSE_MAGIC){
		if($source instanceof EntityDamageEvent && $source instanceof EntityDamageByEntityEvent){
			...
			if($player->getName() == $damager->getName()){
				...
				if($i->getID() == 353 && $this->networkId !== $i->getDamage()){
					$this->networkId = $i->getDamage();
 					$this->despawnFromAll();
					if(!$this->dead){
						$this->spawnToAll();
					}
					$player->sendMessage("[MobPet] 몹 변신 !");
				}else{
				$this->isFallow = !$this->isFallow;
 				$player->sendMessage("[MobPet] ".($this->isFallow ? "잘 따라다닐게요 !" : "여기서 기다릴게요 !"));
				}
			}else{
				$damager->sendMessage("[MobPet] 제 주인은 ".$player->getName()."님입니다. 때리지마요 !");			
				$player->sendMessage("[MobPet] ".$damager->getName()."님이 저 때려요 !");
			}
		}
	}
