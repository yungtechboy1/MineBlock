...
class Money extends PluginBase implements Listener{
		...
		switch(strtolower($cmd->getName())){
			...
			case "moneyop":
				...
				switch(strtolower($sub[0])){
					...
					case "닉네임":
						$mn["Nick"] = !$mn["Nick"];
						$m = $mm . ($ik ? "닉네임 모드를 " . ($mn["Nick"] ? "켭": "끕") . "니다.": "MickName mode is " . ($mn["Nick"] ? "On": "Off"));
						$c = true;
					break;
					case "op":
					case "o":
	...
	public function onTick(){
		...
		foreach($mn as $k => $v){
			...
			$n = $p->getDisplayName();
			$nt = $this->mn["Nick"] ? str_replace(["%name","%rank","%money"], [$n,$this->getRank($k),$v], $this->mn["Nick_Format"]): $n;
	...
	public function loadYml(){
		...
		$this->mn = (new Config($this->getServer()->getDataPath() . "/plugins/! MineBlock/" . "Money.yml", Config::YAML, ["Money" => [],"Default" => "10000","Nick" => true,"Nick_Format" => "[%rank] %name : %money","OP" => true]))->getAll();
	}
